import { InputAmount } from '@burrbear/sdk'
import {
...
import { Pool, PoolToken } from '../pool/pool.types'
import { ApiToken, HumanTokenAmountWithAddress, TokenBase } from './token.types'
  ...
  poolTokens.forEach(poolToken => {
    if (poolToken.nestedPool) {
      ...
      const nestedLeafTokens = nestedTokens.map(t => {
        const token = getTokenOrUnderlying(t)
        return {
          ...token,
          chain: poolToken.chain,
          chainId: poolToken.chainId,
        }
      })
