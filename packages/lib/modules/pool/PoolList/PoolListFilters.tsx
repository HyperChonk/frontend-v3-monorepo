...
import { getChainShortName, isProd } from '@repo/lib/config/app.config'
import { isHyperChonk, PROJECT_CONFIG } from '@repo/lib/config/getProjectConfig'
...
export function PoolNetworkFilters({
  ...
  const buttonProps = isHyperChonk
    ? {
        border: '1px solid',
...
export function PoolListFilters() {
  ...
  const poolCreatorUrl = isHyperChonk
