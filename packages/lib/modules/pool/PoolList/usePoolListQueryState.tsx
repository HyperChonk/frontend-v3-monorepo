...
export function usePoolListQueryState() {
  ...
  const projectPoolTypes = (poolTypes.length > 0 ? poolTypes : Object.keys(POOL_TYPE_MAP)).filter(
    poolType => !PROJECT_CONFIG.options.hidePoolTypes.includes(poolType as GqlPoolType)
  )

  const mappedPoolTypes = uniq(
    projectPoolTypes.map(poolType => POOL_TYPE_MAP[poolType as keyof typeof POOL_TYPE_MAP]).flat()
  )

  const mappedPoolTags = poolTags.length > 0 ? poolTags : []

  const testPoolIds = isProd ? PROJECT_CONFIG.options.testPoolIds || [] : []
