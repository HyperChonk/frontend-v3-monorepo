    ...
    <HStack fontWeight="medium" spacing="lg" {...props}>
      {appLinks.map(link => (
        <Box as={motion.div} key={link.href} variants={fadeIn}>
          <Link
            as={NextLink}
            color={isHyperChonk && isActive(link.href) ? 'hyperTeal' : ''}
            href={link.href}
            isExternal={link.isExternal}
            prefetch
            variant="nav"
            backgroundColor={isActive(link.href) ? 'background.level1' : 'transparent'}
            style={{
        ...
        <>
          <Box as={motion.div} variants={fadeIn}>
            <Link
              ...
              prefetch
              variant="nav"
              backgroundColor={isActive('/debug/pools') ? 'background.level1' : 'transparent'}
            >
              Test-Pools
            </Link>
          </Box>
          <Box as={motion.div} variants={fadeIn}>
            <Link
              ...
              prefetch
              variant="nav"
              backgroundColor={isActive('/debug') ? 'background.level1' : 'transparent'}
            >
              Debug
            </Link>
          </Box>
  ...
  return (
    <Box
      _before={{
        ...
        right: 0,
        bottom: 0,
        bg: leftSlot ? 'transparent' : 'background.navbar',
        zIndex: -1,
      }}
      as={motion.div}
